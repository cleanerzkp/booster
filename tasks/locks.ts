...
const accounts = [
  "0x8465b78e3308bcde4b72ab9b3cd1a51bec954f01",
...
task("locks", "Prints the list of token locks", async (taskArgs, hre) => {
  ...
  for (let index = 0; index < accounts.length; index++) {
    ...
    console.log("Account " + account + " has " + locks.length + " locks");

    const cleanLocks = [];

    for (let lIndex = 0; lIndex < locks.length; lIndex++) {
      cleanLocks.push({
        amount: ethers.utils.formatEther(locks[lIndex].amount),
        duration: locks[lIndex].duration / 60 / 60 / 24 + " days",
        lockedAt: locks[lIndex].lockedAt,
        expiresAt: locks[lIndex].lockedAt + locks[lIndex].duration,
      });
    }

    console.table(cleanLocks);
